...
#include <cmath>
#include "Spring.hpp"
#include "Node.hpp" //Need this to access << for Node

Spring::Spring(Node& n1, Node& n2, double K_1, double l0_1)
{
    K = K_1;
    l0 = l0_1;
    node1 = &n1;
...
bool Spring::is_spring(Node &n1, Node &n2)
{
    return (n1.has_connection(n2) && n2.has_connection(n1));
}

Spring Spring::add_spring(Node &n1, Node &n2, double K_1, double l0_1)
{
    
    if (Spring::is_spring(n1, n2))
    {
        throw std::runtime_error("Cannot create Spring -- nodes are already connected!");
    }
...
void Spring::remove_spring(Node &n1, Node &n2)
{
    if (!Spring::is_spring(n1, n2))
    {
...
Spring Spring::get_spring(Node &n1, Node &n2)
{
    if (!Spring::is_spring(n1, n2))
    {
...
std::ostream& operator<<(std::ostream& os, Spring& s)
{
    ...
    return os << "Spring with Stiffness: " << s.get_stiffness() << "; Rest Length: " << s.get_rest_length() << "; Nodes:\n" << "    " << *s.node1 << "\n" << "    " << *s.node2 << std::endl;
}
